...
function MachineModal({ tile, onClose, preview = false, containerMode = false }: { tile: TileData, onClose: () => void, preview?: boolean, containerMode?: boolean }) {
    const title = preview ? "Equipment Preview" : tile.equipment.name;
    const description = preview
        ? "Placeholder text for equipment details. The full map shows complete descriptions."
        : tile.equipment.description;
    const benefits = preview
        ? ["Exercise A", "Exercise B", "Exercise C"]
        : tile.equipment.benefits;
    const musclesTargeted = preview
        ? ["Muscle Group 1", "Muscle Group 2"]
        : tile.equipment.musclesTargeted;
    const videoUrl = preview ? undefined : tile.equipment.videoUrl;
    return (
        <div className={`${containerMode ? 'absolute' : 'fixed'} inset-0 flex items-center justify-center z-40 cursor-pointer select-none`} onClick={onClose}>
            ...
            >
                ...
                <h1 className="text-xl sm:text-2xl md:text-3xl select-none text-white flex-shrink-0 pr-10">{title}</h1>
                ...
                <div className="grid grid-cols-1 gap-4 sm:gap-6 mt-4 sm:mt-6 md:grid-cols-2 xl:grid-cols-3 xl:grid-rows-2 flex-1 min-h-0 overflow-y-auto">
                    <div className="flex flex-col row-span-1 md:row-span-2 items-center justify-center bg-black/20 rounded-lg p-3 sm:p-4 min-h-0">
                        ...
                        <div className="w-full mt-4 p-2 text-white overflow-y-auto flex-1 min-h-0">
                            ...
                            <p className="select-none">{description ?? "No description available."}</p>
                        </div>
                    </div>
                    ...
                    <div className="rounded-lg p-4 overflow-y-auto text-white bg-black/20 min-h-0">
                        ...
                        <ul className="list-disc list-outside pl-5 space-y-2">
                            {musclesTargeted?.map((muscle: string, idx: number) => (
